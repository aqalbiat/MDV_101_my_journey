...
MODULES.append("instruction_fetch")
MODULES.append("bitty_core")
MODULES.append("bitty_test")

# The name of the modules which has dpi code with it
# Once it is added a filename with structure {name}_dpi.cpp will be searched for compilation
DPI_MODULES = []
DPI_MODULES.append("bitty_test")
...
def compile_all(should_gen_wave):
    ...
    for module in MODULES:
        additional = ""
        if module in DPI_MODULES:
            additional += "  ./test/{1}/{1}_dpi.cpp"
        os.system((VERILATE_COMMAND + additional).format(trace, module))
