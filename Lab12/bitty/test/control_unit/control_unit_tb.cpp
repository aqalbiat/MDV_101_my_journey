...
vector <string> generate_instructions() {
	...
	for (int i = 0; i < 20; i++) {
		...
		int alu_s = abs(rand()) % 8;
		result.push_back(CreateInstruction(rx, ry, alu_s));
	}
	return result;
}
...
int main(int argc, char **argv) {
	...
	signals.push_back(ControlUnitSignals(CreateInstruction(0, 0, 0), false, true, true));
	signals.push_back(ControlUnitSignals(CreateInstruction(0, 0, 0), false, false, true));
