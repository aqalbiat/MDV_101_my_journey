...
module control_unit(
	...
	always @(*) begin
		if (next_state == state1) begin 
			...
			mux_sel = 3'b000;     // setting up everything to default value
			sel = 4'b0000;        // setting up everything to default value
			mode = 1'b0;          // setting up everything to default value
		end else if (next_state == state2) begin
			en_s = 1'b1;
			mux_sel = instructions[15:13];
			/* ---------------------- */
			en_i = 1'b0; // clearing previous signal
		end else if (next_state == state3) begin
			mux_sel = instructions[12:10];
			en_c = 1'b1;
			sel = instructions[6:3];
			mode = instructions[2];
			/* ---------------------- */
			en_s = 1'b0; // clearing previous signal
		end if (next_state == state4) begin
			done = 1'b1;
