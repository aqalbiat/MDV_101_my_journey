...
import "DPI-C" function shortint fetch_next_instruction();
import "DPI-C" function void simulate_current_instruction();
import "DPI-C" function void generate_all_instructions();
...
module bitty_test (
    ...
    always @(posedge clk) begin
        ...
        if (reset) begin
            ...
            if (clock_count % 50000 == 0 || clock_count == 4000029) begin
            ...
            if (next_state == state3) begin
                simulate_current_instruction();
            end
            if (next_state == state1 && clock_count > 1) begin
                if (clock_count < 30) begin
                    PrintModuleState(); 
                end
    ...
    always @(*) begin
        if (current_state == state0 && run) begin
            next_state = state1;
